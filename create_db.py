...
def main():
    ...
    is_data_base_exists = os.path.isfile(database_name)
    ...
    with connection_to_database:  # this promises to close the connection when we're done with it
        ...
        if not is_data_base_exists:  # if database doesn't exists, we're going to create it
    ...
    for line in input:
        ...
        elif line[0] == "S":  # student
            cursor.execute("INSERT INTO students("
                           "grade, count)"
                           "VALUES(?,?)"
                           "list[1], list[2]")
        else:  # line[0] == "R" classroom
